...
uint getWeightsOffset(uvec3 coord){
    ...
    uint localIndex = modula(newCoord.x,2);
    uint maxVal = 65535;
    return maxVal & (weightsBuffer.weights[index(newCoord) / 2] >> (16 * localIndex));
}
const uint iso = 32768;
vec3 interpolate(vec3 x,vec3 y,uint xv, uint yv){
    float t = float(int(iso) - int(xv)) / (int(yv) - int(xv));
    return mix(x,y,vec3(t,t,t));
}
void main(){
    ...
    for (int i = 0; edges[i] != 12 && i < 15; i +=3){
        ...
        float arr[9];
        vec3 v0 = interpolate(cornerOffsets[e00],cornerOffsets[e01],cubeValues[e00],cubeValues[e01]) + offset;
        arr[0] = v0.x;arr[1] = v0.y;arr[2] = v0.z;
        vec3 v1 = interpolate(cornerOffsets[e10],cornerOffsets[e11],cubeValues[e10],cubeValues[e11]) + offset;
        arr[3] = v1.x;arr[4] = v1.y;arr[5] = v1.z;
        vec3 v2 = interpolate(cornerOffsets[e20],cornerOffsets[e21],cubeValues[e20],cubeValues[e21]) + offset;
